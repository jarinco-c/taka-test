...
古い紙に書かれた手書きの日本語（ひらがな・カタカナ・漢字・数字・英字）を正確に読み取ってください。

以下の点に注意してください：
- 文字が不鮮明・かすれている場合は、文脈から最も適切な文字を推定してください
- 読み取れない文字は「□」で表してください
- 縦書き・横書きどちらにも対応してください
- 旧字体・変体仮名も可能な限り現代の表記に変換してください
...
def recognize_page(image_bytes: bytes, page_num: int, hint: Optional[str]) -> str:
    """Send a single page image to Claude and return the recognized text."""
...
async def recognize(
    ...
    pages: Optional[str] = Form(None),  # e.g. "1,3,5" or "1-3" or empty = all
):
    ...
    if pages and pages.strip():
        ...
        # Clamp to valid range (1-indexed input → 0-indexed)
        selected = [p for p in selected if 1 <= p <= total_pages]
    ...
    for page_num in selected:
        text = recognize_page(images[page_num - 1], page_num, hint)
